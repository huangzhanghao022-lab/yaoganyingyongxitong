...
async function autoFillTransferInfo(row: TelecontrolRecord) {
	...
	const result = applyTransferAutoFill(telemetryText, imagingBlocks, AS03_FILE_GROUP_SIZE);
	if (result.updated === telemetryText) {
		ElMessage.info(t("无需补齐"));
		return;
	}
	try {
		await service.daily_plan.as03.update({ id: row.id, telemetryInfo: result.updated });
...
async function fetchDailyPlanRecords(): Promise<TelecontrolRecord[]> {
	...
	for (;;) {
	const res = await service.daily_plan.as03.page({ page, size: pageSize });
