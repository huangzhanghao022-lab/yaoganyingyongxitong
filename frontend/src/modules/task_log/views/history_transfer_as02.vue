...
defineOptions({
	name: "task_log-history_transfer_as02",
});
...
const Table = useTable({
	columns: [
		...
		{ label: t("记录文件号"), prop: "recordFileNo", minWidth: 120 },
		{
			label: t("开始日期"),
			prop: "startDate",
			minWidth: 140,
...
const Crud = useCrud(
	{
		service: service.task_log.history_transfer_as02,
		onDelete(selectionRows) {
			...
			return service
				.request({
					...
					method: "POST",
					data: { satellite: "AS02", type: "history_transfer", times },
				})
				.then(() => {
					// @ts-ignore
