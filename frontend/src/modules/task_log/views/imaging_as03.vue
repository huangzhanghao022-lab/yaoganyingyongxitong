...
const Table = useTable({
	columns: [
		...
		{ label: t('文件号'), prop: 'startFileNo', minWidth: 120, sortable: 'custom' },
...
const Crud = useCrud(
	{
		service: service.task_log.imaging_as03,
		onDelete(selectionRows) {
			const rows = Array.isArray(selectionRows) ? selectionRows : selectionRows ? [selectionRows] : [];
			const times = rows.map((row: any) => row?.imagingTime).filter((time: any) => !!time);
			if (!times.length) {
				// @ts-ignore
			...
			return service
				.request({
					...
					method: 'POST',
					data: { satellite: 'AS03', type: 'imaging', times },
				})
				.then(() => {
					// @ts-ignore
